...
npm run build:cli                        # build the `appe` CLI (see below)
...
The same estimate, without a browser:

```console
$ npm run build:cli
$ npx appe estimate "summarise a customer support ticket" --count 10000 --top 3

Task  summarise a customer support ticket
Each item  9 input + 500 output tokens  (output assumed — set --output-tokens or --output)
Run  10,000 items  ·  5,090,000 tokens total  (~5 book series (10 books each))

#  MODEL                          PROVIDER  TIER   $/MTOK IN  $/MTOK OUT  COST/ITEM  TOTAL × 10,000
1  Voxtral Small 24B              evroc     small      0.002       0.002  $0.000001         $0.0117
2  Meta Llama Prompt Guard 2 22M  helicone  small      0.010       0.010  $0.000005         $0.0509
3  Meta Llama Prompt Guard 2 86M  helicone  small      0.010       0.010  $0.000005         $0.0509

Ranked 4,737 of 5,260 models · showing top 3 · 523 with no output price hidden (--include-free)
Cheapest Voxtral Small 24B at $0.0117 · dearest o1-pro at $3,014 · spread ×257,410
```

`--json` makes it composable (`appe estimate "classify a review" -n 1e6 --json |
jq '.results[0]'`); `--provider`, `--tag`, `--tier` and `--top` narrow the field.
It imports `@appe/core`, so a CLI number and a web number for the same inputs are
the same number by construction. Full docs: [`packages/cli/README.md`](packages/cli/README.md).
...
```
packages/
  core/           @appe/core — the estimator, framework-free. computations.ts,
    src/          imageCost.ts, tokenization/, format.ts, types/, and the
                  generated catalogue in data/ (ALL_MODELS, ALL_PROVIDERS,
                  ALL_TAGS, MODELS_META). Imported by BOTH consumers below.
  cli/            the `appe` command — estimate.ts (ranking + filters),
    src/          format.ts (table rendering), index.ts (arg parsing).
                  Bundled by esbuild into dist/appe.js.
src/              the web app — the other consumer of @appe/core
  lib/            urlConfig.ts (share-link (de)serialization), utils
...
Anything that computes a token count or a cost belongs in `packages/core`;
anything that renders belongs in `src/` (browser) or `packages/cli/` (terminal).
That is what keeps the two front-ends from ever quoting different prices.
...
item is always the best thing to pick up. Near-term: agentic cost estimation
(turns, growing context, cache hits), and `appe models` for searching the
catalogue from the terminal.
